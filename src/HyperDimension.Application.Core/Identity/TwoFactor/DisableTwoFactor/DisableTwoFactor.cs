...
public class DisableTwoFactor : IRequest<IActionResult>
{
    [FromBody]
    public DisableTwoFactorBody Body { get; set; } = new();
}

public class DisableTwoFactorBody
{
...
public class DisableTwoFactorHandler : IRequestHandler<EnableTwoFactor.EnableTwoFactor, IActionResult>
{
    private readonly ITotpService _totpService;
    private readonly IHyperDimensionRequestContext _requestContext;
    private readonly IStringLocalizer<DisableTwoFactorHandler> _localizer;
    ...
    public DisableTwoFactorHandler(
        ...
        IStringLocalizer<DisableTwoFactorHandler> localizer,
    ...
    public async Task<IActionResult> Handle(EnableTwoFactor.EnableTwoFactor request, CancellationToken cancellationToken)
    {
        ...
        {
            case "totp" when user.TwoFactorTotpEnabled is false:
                return new ErrorMessageResult(_localizer["Two factor authentication with TOTP is already disabled."]).ToBadRequest();
            case "totp":
            {
                ...
                user.TwoFactorTotpEnabled = false;
                break;
            }
            case "email" when user.TwoFactorEmailEnabled is false:
                return new ErrorMessageResult(_localizer["Two factor authentication with email is already disabled."]).ToBadRequest();
            case "email":
            {
